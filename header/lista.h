...
template<class T>
ostream & operator<<(ostream & salida,const Lista<T> & l);

template<class T>
ostream & operator<<(ostream & salida,const Lista < pair < T, T > > &l);
...
template<class T> void Lista<T>::mostrar(ostream & salida) const
{
	salida << "[";
	if (longitud() > 0) {
		salida << iesimo(0);
